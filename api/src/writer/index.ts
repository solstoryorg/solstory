 ...
 */
export class SolstoryWriterAPI {
  ...
  solstoryItemToUpdateHeadData(solItem: SolstoryItemContainer, objId:Uint8Array, accessType:AccessType): UpdateHeadData{
      const cleanData = {
          timestamp: new anchor.BN(solItem.validated.timestamp),
          dataHash: Uint8Array.from(Buffer.from(solItem.validated.itemHash, 'hex')),
          currentHash: Uint8Array.from(Buffer.from(solItem.validated.nextHash, 'hex')),
  ...
  async appendItem(mintId: PublicKey, item:SolstoryItemInner, options: SolstoryAppendItemOptions = {confirmation:{}}){
    ...
    const fullItem:SolstoryItemContainer = {
      validated: {
        ...
        item: item,
        itemHash: dataHash,
        nextHash: Buffer.from(headAct.currentHash).toString('hex'),
        timestamp: timestamp,
      },
